    ...
    public boolean menuSelected(MenuItem menuItem) {
        switch (menuItem.getItemId()) {
            case R.id.menuMedia:
                ActivitiesCounter
                        .getInstance()
                        .addActivity(new DoubleViewEntity(Constants.FROM_NAVIGATION_VIEW, "to Media"));
                mView.switchToFragment(new MediaFragment());
                return true;
            case R.id.menuUserActivities:
                ActivitiesCounter
                        .getInstance()
                        .addActivity(new DoubleViewEntity(Constants.FROM_NAVIGATION_VIEW, "to Activities"));
                mView.switchToFragment(new UserActivitiesFragment());
                return true;
            case R.id.menuSocket:
                ActivitiesCounter
                        .getInstance()
                        .addActivity(new DoubleViewEntity(Constants.FROM_NAVIGATION_VIEW, "to Socket"));
